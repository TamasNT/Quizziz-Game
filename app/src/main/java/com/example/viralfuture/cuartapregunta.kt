...
class cuartapregunta : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_cuartapregunta)
        ...
        countDownTimer = object : CountDownTimer(timeLeftInMilliseconds, 1000) {
            ...
            override fun onFinish() {
                nombre = EditableText.text.toString()
                    ...
                    val i = Intent(applicationContext, quintapregunta::class.java)
                    startActivity(i)
            }
        }
    ...
    fun pista(view: View) {
        ...
        if(view.id == R.id.pista){
            ...
            Toast.makeText(applicationContext,"Empieza con J y termina con S", Toast.LENGTH_SHORT).show()
        }
    }

    // Metodo para cambiar de vista al presionar el boton
    fun GoToQuintaPregunta (view: View){
        nombre = EditableText.text.toString()
        if(nombreenviado){
            if(nombre.lowercase()=="jarvis") {
                BaseDatos.Correctas++
