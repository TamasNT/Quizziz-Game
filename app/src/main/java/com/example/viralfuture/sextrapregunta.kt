...
class sextrapregunta : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sextrapregunta)
        ...
        countDownTimer = object : CountDownTimer(timeLeftInMilliseconds, 1000) {
            ...
            override fun onFinish() {
                if (Opcion1.isChecked && Opcion3.isChecked) {
                    BaseDatos.Correctas++
                    BaseDatos.Puntuacion++
                }else{
                    BaseDatos.Incorrectas++
                }
                val i = Intent(applicationContext, septimapregunta::class.java)
                startActivity(i)
            }
        }
    ...
    fun GoToSeptimaPregunta (view: View){
        if (Opcion1.isChecked && Opcion3.isChecked) {
            BaseDatos.Correctas++
            BaseDatos.Puntuacion++
        }else{
